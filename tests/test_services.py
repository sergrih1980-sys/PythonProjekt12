...
class TestSimpleSearch(unittest.TestCase):
    ...
    def test_simple_search_multiple_matches(self):
        ...
        result = simple_search("аз", self.test_transactions)  # должно найти 'АЗС' и 'Кафе'
        response = json.loads(result)
        ...
        self.assertIn(4, found_ids)  # Бензин на АЗС
    ...
    def test_simple_search_whitespace_query(self):
        ...
        self.assertEqual(response["status"], "success")
        self.assertEqual(response["search_query"], "")  # нормализация уберёт пробелы
        self.assertEqual(response["found_count"], 0)
